...
// ---------------------------------------------------------------------------
// Caching Synced Original Lyrics Fetched from YTM for a Song with Missing Lyrics
// ---------------------------------------------------------------------------
...
// YouTube Music is tried first: if it finds synced (LRC-timestamped) lyrics
// the result is stored in LyricsCache with:
//
//   nativeStatus : 'MISSING'   ← Spotify's original state (no lyrics)
//   prefetchState: 'SYNCED'    ← what YTM provided
//   data.isSynced: true
//   data.source  : 'YouTube Music'
//
// When YTM finds nothing at all the engine returns { ok:false } — a failure
...
//
// Scenarios tested:
//   1.  A YTM synced result is stored for a MISSING-tagged track.
...
//   6.  A failed result (ok:false, no data) is cached to block re-fetch loops.
//   7.  A cached failure is distinguishable from an unsynced+plain entry.
//   8.  A MISSING-failed entry and a MISSING-synced entry for different tracks
//       coexist without interfering.
//   9.  Cache key normalisation is consistent.
...
/** YTM successfully found synced lyrics for a track Spotify reported as MISSING. */
...
describe('Caching Synced Original Lyrics from YTM for a Song with Missing Lyrics', () => {
    ...
    // -------------------------------------------------------------------------
    it('stores a YTM synced result for a track Spotify reported as MISSING', () => {
        const key = 'spotify:track:missing01';
    ...
    // -------------------------------------------------------------------------
    it('serves the cached YTM synced result without re-fetching', () => {
        const fetchFn = vi.fn().mockResolvedValue(makeMissingToSyncedResult());
        const key = 'spotify:track:missing02';
    ...
    it('preserves the isSynced:true flag through the cache round-trip', () => {
        const key = 'spotify:track:missing03';
        cache.set(key, makeMissingToSyncedResult());
    ...
    it('preserves the prefetchState SYNCED field recording the upgrade from MISSING', () => {
        const key = 'spotify:track:missing04';
    ...
    // -------------------------------------------------------------------------
    it('preserves the nativeStatus MISSING marker alongside the YTM synced lyrics', () => {
        const key = 'spotify:track:missing05';
        cache.set(key, makeMissingToSyncedResult());
        const hit = cache.get(key);
    ...
    it('caches a failed result (ok:false) when all sources return nothing', () => {
        const key = 'spotify:track:missing06';
    ...
    it('serves a cached failure result without re-fetching (blocks re-fetch loops)', () => {
        ...
        const key = 'spotify:track:missing07';
    ...
    it('distinguishes a MISSING+failed entry from an UNSYNCED+plain entry', () => {
        const missingKey  = 'spotify:track:missing-track';
        const unsyncedKey = 'spotify:track:unsynced-track';
    ...
    it('keeps a MISSING+failed entry and a MISSING+synced entry independent', () => {
        const failedKey = 'spotify:track:truly-missing';
        const savedKey  = 'spotify:track:missing-but-ytm-found';
    ...
    it('prefers the Spotify URI key over the title|artist fallback key', () => {
        const uri      = 'spotify:track:uniqueuri333';
    ...
    it('evicts the least-recently-used entry when the cache exceeds its limit', () => {
        ...
        // song1 → t=5000, song2 → t=5100, …, song5 → t=5400
    ...
    it('tracks an in-flight promise and removes it once the fetch resolves', async () => {
        const key = 'spotify:track:inflight-missing';
